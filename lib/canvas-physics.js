...
window.createLoopSimulation = function createLoopSimulation(canvas, opts) {
  ...
  function setHeight(h) {
    ...
    failReason = '';
    buildTrack(heightR);
    ballS = 0; ballV = 0; ballX = 0; ballY = 0; ballVx = 0; ballVy = 0; trail = [];
    render();
  }
  ...
  function start() {
    ...
    failReason = '';
    buildTrack(heightR);
    ballS = 0; ballV = 0; ballX = 0; ballY = 0; ballVx = 0; ballVy = 0; trail = []; lastTime = 0;
  ...
  function step(timestamp) {
    ...
    if (mode === 'freefall') {
      // Pure projectile motion: gravity acts downward (canvas +y).
      ballVy += gravity * dt;
      ballX += ballVx * dt;
      ballY += ballVy * dt;
      trail.push({ x: ballX, y: ballY });
      if (trail.length > 120) trail.shift();
      // Stop once it leaves the canvas (treat any edge as "the ground").
    ...
    // Loss-of-contact check on the upper half of the loop. Required centripetal
    // force at angle θ (canvas convention, sin θ < 0 = above center) is
    // m·v²/R; gravity contributes -sin θ · g toward the center. When v²/R is
    // less than that, the track would have to *pull* on the ball — but this is
    // a one-sided rail, so the ball loses contact and switches to projectile
    // motion.
    if (ballS > loopStartS && ballS < loopEndS) {
      ...
      if (sinT < 0) {
        ...
        if (ballV * ballV < required) {
          // Detach: hand off arc-length velocity to a 2D velocity vector
          // along the spline tangent at the release point.
          const tan = spline.tangentAt(ballS);
  ...
  function render() {
    ...
    if (mode === 'simulate' || mode === 'success' || mode === 'fail' || mode === 'freefall') {
      const onTrack = mode !== 'freefall';
      const pos = onTrack ? spline.positionAt(ballS) : { x: ballX, y: ballY };
      ctx.beginPath();
      ctx.arc(pos.x, pos.y, ballRadius, 0, Math.PI * 2);
      ctx.fillStyle = ballColor;
      ctx.fill();
    ...
    if (mode === 'success') {
      ...
      ctx.font = 'bold 16px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText('Loop completed!', w / 2, h - 13);
      ctx.textAlign = 'left';
    }
  }
