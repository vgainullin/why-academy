...
# Target-level resume: if this target already completed (ACCEPTED.txt or
# FAILED.txt + target_metrics.json), exit early without re-running.
if [[ -f "$TARGET_DIR/target_metrics.json" ]] &&
	{ [[ -f "$TARGET_DIR/ACCEPTED.txt" ]] || [[ -f "$TARGET_DIR/FAILED.txt" ]]; }; then
	echo "[evolve_target] SKIP: target already completed (target_metrics.json present)" >&2
	cat "$TARGET_DIR/target_metrics.json"
	exit 0
fi

# Write batch-level checkpoint on first target encountered (race-free enough for our use)
if [[ ! -f "$EVO_BASE/checkpoint.json" ]]; then
	"$PY" - "$EVO_BASE/checkpoint.json" "$BATCH_ID" "$MAX_ITER" "$INNER_ENGINE" "$INNER_MODEL" "$JUDGE_ENGINE" "$JUDGE_MODEL" "$EVOLVE_ENGINE" "$EVOLVE_MODEL" <<'PYEOF'
...
"$PY" - "$TARGET_DIR/target.json" "$BATCH_ID" "$TARGET_INDEX" "$TARGET" <<'PYEOF'
import json, datetime, sys
...
for ((ITER = 0; ITER < MAX_ITER; ITER++)); do
	...
	# Iter-level resume: if this iter already has a terminal status, honor it.
	if [[ -f "$ITER_DIR/status.txt" ]]; then
		PRIOR_STATUS="$(cat "$ITER_DIR/status.txt")"
		case "$PRIOR_STATUS" in
		PASS)
			ACCEPTED_ITER="iter_$(printf '%02d' "$ITER")"
			echo "[evolve_target] resume: iter $ITER already PASS" >&2
			break
			;;
		verify_fail | canvas_fail | problem_missing | evolve_fail | *_fail_exit_*)
			FAIL_REASON="${PRIOR_STATUS}_iter_$ITER"
			echo "[evolve_target] resume: iter $ITER already terminal ($PRIOR_STATUS)" >&2
			break
			;;
		esac
	fi
	...
	# Move any stale canonical artifacts aside (non-destructive) instead of rm -f.
	# This preserves evidence from prior runs while keeping the path clear.
	PRIOR_DIR="$ITER_DIR/.prior"
	for stale in "$ROOT/derivations/problems/${PROBLEM_ID}".*; do
		[[ -f "$stale" ]] || continue
		mkdir -p "$PRIOR_DIR"
		mv "$stale" "$PRIOR_DIR/$(basename "$stale")"
	done

	# Render <<TARGET>> + <<PROBLEM_ID>>
	"$PY" - "$ITER_DIR/variant.md" "$TARGET" "$PROBLEM_ID" "$ITER_DIR/rendered_prompt.md" <<'PYEOF'
import sys
...
# target_metrics.json
"$PY" - "$TARGET_DIR" "$TARGET_INDEX" <<'PYEOF'
import json, sys
from pathlib import Path
td = Path(sys.argv[1])
