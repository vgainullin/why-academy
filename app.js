...
(function () {
  ...
  const ensureSympy = C.ensureSympy;
  const canonicalizeLatex = C.canonicalizeLatex;
  const transcribeHandwriting = C.transcribeHandwriting;
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    C.startPyodidePreload(sympyNeededCallback);

    function sympyNeededCallback() {
      if (sympyNeeded()) {
  ...
  function sympyNeeded() {
    ...
    return lesson.blocks.some(function (b) { return b.type === 'handwrite' || b.type === 'canvas-derive'; });
  }
...
plt.close('all')
          ...
          C.pyodide.runPython(code);

          // Capture stdout
          const stdout = C.pyodide.runPython('sys.stdout.getvalue()');
          const stderr = C.pyodide.runPython('sys.stderr.getvalue()');
          outputDiv.textContent = stdout + (stderr ? '\n' + stderr : '');
          if (!stdout && !stderr) outputDiv.textContent = '(no output)';

          // Capture plot if any
          const hasPlot = C.pyodide.runPython(`
import matplotlib.pyplot as plt
...
plt.close('all')
            `);
            const b64 = C.pyodide.runPython('_plot_b64');
            ...
            block.checks.forEach(check => {
              ...
              try {
                if (check.variable && check.expected_expr) {
                  const actual = C.pyodide.runPython(check.variable);
                  const expected = C.pyodide.runPython(check.expected_expr);
  ...
  function renderHandwrite(block) {
    ...
    function renderStep(stepIdx) {
      ...
      const dc = new DrawingCanvas(canvas, { inkColor: '#000' });
      ...
      clearBtn.addEventListener('click', () => dc.clear());
      ...
      submitBtn.addEventListener('click', async () => {
        if (busy) return;
        if (dc.strokes.length < 1) {
          statusEl.textContent = 'Draw something first.';
          return;
        }
        const dataUrl = dc.getDataUrl(20);
        if (!dataUrl) {
      ...
      async function verify(studentLatex) {
        const targetLatex = step.target_latex || step.result_equation;

        // Steps marked as 'axiom' or 'naming' can't be checked symbolically.
        // Accept if the student wrote something non-trivial.
        if (step.sympy_check === 'axiom' || step.sympy_check === 'naming') {
          if (studentLatex && studentLatex.trim().length > 0) {
            feedbackEl.innerHTML =
        ...
        const targetCanon = canonicalizeLatex(targetLatex);
        let verdict, detail;
        try {
          C.pyodide.globals.set('_s', studentCanon);
          C.pyodide.globals.set('_t', targetCanon);
        ...
        if (remaining > 0) {
          ...
          feedbackEl.innerHTML =
            ...
            '<div class="handwrite-reveal">The target was: $$' + targetLatex + '$$' +
  ...
  function evalFormula(formula, params) {
    return C.evalFormula(formula, params);
  }
